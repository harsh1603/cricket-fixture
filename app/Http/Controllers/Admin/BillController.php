...
class BillController extends Controller
{
    public function index()
    {
        ...
        $bills = Bills::whereStatus(1)->where('bill_type',1)->orderByDesc('id')->get();
    ...
    public function show(Request $request,$id)
    {
        ...
        return view('admin.bill.productlist', compact('bills'));
    ...
    public function saveBills(Request $request){
        try{
           // dd($request->all());
            abort_unless(\Gate::allows('bill_create'), 403);
            $invoice_no = Bills::select('invoice_count_no')->where('bill_type',1)->orderBy('id', 'Desc')->first();
            $num=0;
            if($invoice_no!=''):
         ...
         $innum = $num+1;
            ...
            return redirect()->route('admin.bill.generateBill',[\Crypt::encrypt($bills)]);
        }catch(Exception $e){
            Alert::error('Error', $e->getMessage());
            \Log::error($e->getMessage());
            abort(404);
    ...
    public function generateBill($id){
       ...
       $setting = \DB::table('setting')->first();
        return view('admin.bill.generatebill',['data'=>$data,'setting'=>$setting]);
		return $pdf->download('invoice.pdf');	
