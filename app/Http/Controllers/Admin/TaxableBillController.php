...
use Auth;
use PDF;
class TaxableBillController extends Controller
{
    public function index()
    {
        abort_unless(\Gate::allows('bill_access'), 403);

        $bills = Bills::whereStatus(1)->where('bill_type',2)->orderByDesc('id')->get();
        return view('admin.taxbill.index', compact('bills'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.taxbill.create');
    }
    ...
    public function show(Request $request,$id)
    {
        ...
        return view('admin.taxbill.productlist', compact('bills'));
    ...
    public function destroy(Request $request,$id)
    {
        ...
        return redirect()->route('admin.taxbill.index');
          }catch(Exception $e){
    ...
    public function saveBills(Request $request){
        try{
          //  dd($request->all());
            abort_unless(\Gate::allows('bill_create'), 403);
            $invoice_no = Bills::select('invoice_count_no')->where('bill_type',2)->orderBy('id', 'desc')->first();
            $num=0;
            if($invoice_no!=''):
           ...
           $innum = $num+1;
            $data = array(
                ...
                'gst_amt'=>$request->cgst_amt+$request->sgst_amt,
            ...
            return redirect()->route('admin.taxbill.generateBill',[\Crypt::encrypt($bills)]);
        }catch(Exception $e){
            Alert::error('Error', $e->getMessage());
            \Log::error($e->getMessage());
            abort(404);
    ...
    public function generateBill($id){
     ...
     //   dd($data);
       $setting = \DB::table('setting')->first();
        return view('admin.taxbill.generatebill',['data'=>$data,'setting'=>$setting]);
		return $pdf->download('invoice.pdf');	
