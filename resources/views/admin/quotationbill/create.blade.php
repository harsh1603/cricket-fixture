...
<div class="card d-none" id="generatebills">
    <div class="card-body">
        <form method="POST" action="{{route('admin.quotationbill.saveBills')}}">
            @csrf
            ...
            <div class="row mt-3">
                <div class="col-md-12 text-center ">
                    <button class="btn btn-success text-center" id="generate" type="sumbmit">Generate Quotation</button>
                </div>
            </div>
        </form>
    </div>
...
function getResult() {
    ...
    $.ajax({
        url: "{{ route('admin.quotationbill.addproduct')}}",
        headers: {
...
function removepro(that) {
    ...
    if (confirm('Are you Sure ?')) {
        ...
        $.ajax({
            url: "{{ route('admin.quotationbill.removeproduct')}}",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: {
